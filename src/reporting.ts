...
function renderHumanReport(report: Report, jsonPath: string) {
  ...
  meta(
    "sha256",
    report.artifact.sha256.slice(0, 16) +
      (report.artifact.sha256.length > 16 ? "..." : ""),
  );
  if (report.intelligence.socket)
    meta("socket", report.intelligence.socket.status);
  if (report.intelligence.osv) {
    const vulnCount = report.intelligence.osv.vulnerabilities?.length ?? 0;
    meta(
      "osv",
      `${report.intelligence.osv.status}${vulnCount ? ` (${vulnCount} advisory${vulnCount === 1 ? "" : "ies"})` : ""}`,
    );
  }
  if (report.intelligence.npmSignature)
    meta("npm-signature", report.intelligence.npmSignature.status);
  if (report.intelligence.typosquat) {
    const matches =
      report.intelligence.typosquat.suspiciousMatches?.length ?? 0;
    meta(
      "typosquat",
  ...
  if (report.intelligence.packageAge?.status === "checked") {
    ...
    const parts: string[] = [];
    if (typeof a.packageAgeDays === "number")
      parts.push(`package=${a.packageAgeDays}d`);
    if (typeof a.versionAgeHours === "number")
      parts.push(`version=${a.versionAgeHours}h`);
  ...
  console.log(
    `  ${c.dim("md  ")} ${c.gray(jsonPath.replace(/\.json$/, ".md"))}`,
  );
}
...
function intelligenceLines(report: Report) {
  ...
  if (socket) {
    ...
    if (components.length)
      lines.push(`  - Socket components: ${components.join(", ")}`);
  } else lines.push("- Socket: not-applicable");
  const osv = report.intelligence.osv;
  lines.push(
    `- OSV: ${osv?.status ?? "not-applicable"}; vulnerabilities=${osv?.vulnerabilities?.length ?? 0}`,
  );
  if (osv?.vulnerabilities?.length)
    lines.push(
      ...osv.vulnerabilities
        .slice(0, 5)
        .map(
          (v) =>
            `  - ${v.severity}: ${v.id}${v.summary ? ` — ${v.summary}` : ""}`,
        ),
    );
  lines.push(
    `- npm signature: ${report.intelligence.npmSignature?.status ?? "not-applicable"}${report.intelligence.npmSignature?.message ? `; ${report.intelligence.npmSignature.message}` : ""}`,
  );
  lines.push(
...
function uncheckedLines(report: Report) {
  const minified = report.findings.filter((f) => f.id.endsWith(".minified"));
  const lines = minified.map(
    (f) =>
      `- Static pattern scanning skipped for minified file: ${f.id.replace(/^file\./, "").replace(/\.minified$/, "")}. Coverage is incomplete until original source is inspected.`,
  );
  if (report.intelligence.socket?.status !== "checked")
    lines.push(
      `- Socket intelligence was not checked (${report.intelligence.socket?.status ?? "missing"}).`,
    );
  if (report.intelligence.osv?.status !== "checked")
    lines.push(
      `- OSV vulnerability lookup was not checked (${report.intelligence.osv?.status ?? "missing"}).`,
    );
  return lines.length
    ? lines
...
export async function maybeRunConfiguredAgentReview(
  report: Report,
  reportPath: string,
  args: string[],
  json: boolean,
